...
class ThermoCorrections(ReactionModelWrapper):
    ...
    def __init__(self, reaction_model=None):
        ...
        defaults = dict(
            ...
            thermodynamic_variables=['temperature',
                                     'gas_pressures', 'voltage', 'beta', 'pH', 'Upzc'],
    ...
    def _get_echem_corrections(self, correction_dict):
        ...
        if any(ads in ['ele_g', 'H_g', 'OH_g'] for ads in self.species_definitions.keys()):
            ...
            G_OH = G_H2O - G_H + self._kB * self.temperature * \
                np.log(self.gas_pressures[H2O_index] * 1.e14)
    ...
    def concentration_pressure(self):
        ...
        self.gas_pressures = [self.species_definitions[g][
            'concentration'] * self.pressure for g in self.gas_names]
    ...
    def approach_to_equilibrium_pressure(self):
        ...
        if sorted(list(set(products)) + list(set(reactants))) != sorted(self.gas_names):
            raise UserWarning(
                'All gasses must appear as products or reactants in self.global_reactions.')
